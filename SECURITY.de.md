...
`swiss-ip-mcp` wurde gegen den internen MCP-Best-Practice-Audit-Katalog
...
Data**. Alle 11 Tools fragen ausschliesslich die IGE/IPI Swissreg Datadelivery API
ab. Bereits umgesetzte Härtungsmassnahmen:

| Bereich | Kontrolle |
|---|---|
| Egress | HTTPS-erzwungene Code-Layer-Allow-List ausschliesslich für `www.swissreg.ch` und das IGE-IDP `idp.ipi.ch`, via `_assert_host_allowed`-Prüfung vor jedem Request (SEC-021) |
| TLS | Zertifikatsverifizierung standardmässig aktiv für alle Upstream-Aufrufe (SEC-005) |
| Binding | Netzwerk-Transporte binden standardmässig an `127.0.0.1`; `0.0.0.0` nur im Container / hinter Reverse-Proxy (SEC-016) |
| Transport | Streamable HTTP mit CORS, das nur `Mcp-Session-Id` exponiert, plus optionale `Host`-Header-Allow-List mit DNS-Rebinding-Schutz (SDK-004) |
| Input | Pydantic-v2-Validierung + XML-Escaping an allen Request-Grenzen (SEC-018) |
| Secrets | Nur Umgebungsvariablen (`IGE_USERNAME` / `IGE_PASSWORD`), `.gitignore` schützt `.env`, `.env.example` liefert Platzhalter, keine hartcodierten Secrets (ARCH-005/SEC-013) |
| Fehler | Maskierte, verständliche Meldungen an das Modell; rohe Upstream-Bodies und Stacktraces werden ausschliesslich nach stderr geloggt (OBS-002) |
| Stdout | Reserviert für den JSON-RPC-Stream; Logging fest auf stderr (OBS-004) |
| Secret-Scanning | Gitleaks-Secret-Scan-Workflow läuft in CI (SEC-013) |
