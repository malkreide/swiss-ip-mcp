...
- **Adressliste ohne Zugangsdaten geprüft (2026-08-08).** Jede Adresse, der Keycloak-Realm und der `client_id`, die dieser Server baut, sind die, die die Quelle führt. Ein Nullbefund — und als solcher aufgezeichnet, denn ein Nullbefund, den niemand notiert hat, ist beim nächsten Durchgang keiner. Drei unabhängige Belege, weil einer nicht getragen hätte:
  - Der IDP unterscheidet: `realms/egov` mit falschen Zugangsdaten → `invalid_grant` («Invalid user credentials»); ein erfundener Realm → 404 «Realm does not exist»; ein erfundener `client_id` → `invalid_client`. Realm und Client existieren also, und es fehlen nur die Zugangsdaten.
  - Der Realm nennt seinen Token-Endpunkt selbst unter `.well-known/openid-configuration` — identisch mit der hier gebauten URL.
  - Die offizielle API-Doku deklariert beide Adressen wörtlich, den `client_id` ausdrücklich als «constant string».
- **Warum es den dritten Beleg braucht:** Die Swissreg-API selbst unterscheidet **nicht**. Ein POST ohne Token gibt 403, mit erfundenem Token 401 — und ein frei erfundener Pfad unter `/public/api/` gibt jeweils dasselbe. Ein Statuscode belegt dort nichts. `scripts/record_fixtures.py` misst das bei jedem Lauf neu und bricht ab, wenn eine Kontrolle nicht mehr unterscheidet.
- **Die Antwort-Payloads sind nicht aufgezeichnet.** Sie brauchen Zugangsdaten; `PROVENANCE.md` führt sie als NICHT AUFGEZEICHNET mit dem gemessenen Statuscode, statt ihnen ein Datum anzuschreiben, das sie nie hatten. Unbelegt bleibt damit die *Form* der Antworten — ob die XML-Felder so heissen, wie der Parser sie liest.
