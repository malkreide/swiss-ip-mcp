...
`swiss-ip-mcp` was hardened against the internal MCP best-practice audit
...
tools only query the IGE/IPI Swissreg Datadelivery API. Hardening already in
place:

| Area | Control |
|---|---|
| Egress | HTTPS-enforced code-layer allow-list to `www.swissreg.ch` and the IGE IDP `idp.ipi.ch` only, via an `_assert_host_allowed` pre-request check (SEC-021) |
| TLS | Certificate verification on by default for all upstream calls (SEC-005) |
| Binding | Network transports default to `127.0.0.1`; `0.0.0.0` only inside a container / behind a reverse proxy (SEC-016) |
| Transport | Streamable HTTP with CORS exposing only `Mcp-Session-Id`, plus optional `Host`-header allow-list with DNS-rebinding protection (SDK-004) |
| Input | Pydantic v2 validation + XML escaping at all request boundaries (SEC-018) |
| Secrets | Env-vars only (`IGE_USERNAME` / `IGE_PASSWORD`), `.gitignore` guards `.env`, `.env.example` ships placeholders, no hardcoded secrets (ARCH-005/SEC-013) |
| Errors | Masked, user-friendly messages to the model; raw upstream bodies and stack traces logged to stderr only (OBS-002) |
| Stdout | Reserved for the JSON-RPC stream; logging pinned to stderr (OBS-004) |
| Secret scanning | Gitleaks secret-scan workflow runs in CI (SEC-013) |
